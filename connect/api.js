...
const getString = (string) => string.toString().trim().toLowerCase();
...
const fetchFoods = () =>
   isLive
      ? fetchValues(foodsEndpoint)
      : Promise.resolve(testFoods.value);

const fetchFoodById = (id) =>
   isLive
      ? fetchValues(`${foodByIdEndpoint}/${id}`)
      : Promise.resolve(testFoods.value.filter(food => getString(food.id) === getString(id)));

const fetchFoodByName = (name) =>
   isLive
      ? fetchValues(`${foodByNameEndpoint}/${name}`)
      : Promise.resolve(testFoods.value.filter(food => getString(food.foodname).includes(getString(name))));
